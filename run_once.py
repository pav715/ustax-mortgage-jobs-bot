...
def main():
    ...
    if not new_jobs:
        ...
        _write_cycle_report(len(jobs), len(india_jobs), len(matched_jobs), 0, 0, len(seen), telegram_ok=tg_ok)
        return
    ...
    save_stats(stats)
    _mark_run_complete(state)
    _write_cycle_report(len(jobs), len(india_jobs), len(matched_jobs), len(new_jobs), sent, len(seen), telegram_ok=tg_ok)
